...
**Google Gemini API** – Required for the ADK agents to function. Get your API key from [Google AI Studio](https://aistudio.google.com/app/apikey).
...
**Note:** The project uses `gemini-2.5-flash` model. 

**Important:** This model may have quota limitations on the free tier. If you encounter a `429 RESOURCE_EXHAUSTED` error:

1. **Check your quota usage** at [Google AI Studio Usage](https://aistudio.google.com/app/usage)
2. **Wait for quota reset** - Free tier quotas typically reset:
   - Per-minute limits: Wait 1 minute
   - Daily limits: Wait until the next day (resets at midnight Pacific Time)
3. **Upgrade your plan** if you need higher limits - Visit [Google AI Studio](https://aistudio.google.com/app/apikey) to upgrade

**Why this model?** The ADK uses API version v1beta, which has limited model support. `gemini-2.0-flash` is one of the models that works with this API version, though it may have stricter quota limits than older models.

**Alternative:** If you continue to hit quota limits, you may need to:
- Use the model less frequently
- Upgrade to a paid plan for higher quotas
- Wait for Google to adjust free tier limits
